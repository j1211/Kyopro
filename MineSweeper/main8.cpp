//今までのあらすじ：一通りできた, プレイヤーの移動をちょっと制限する（パックマン風にしない）
#include <stdio.h>
#include <conio.h>
#include <windows.h>
#include <stdlib.h>
...
public:
	...
	void move() {
		...
		//↑→↓← = wdsa
		if (c == 'w') { y = max(0, y - 1);      move_cnt++; }
		if (c == 'd') { x = min(w - 1, x + 1);  move_cnt++; }
		if (c == 's') { y = min(h - 1, y + 1);  move_cnt++; }
		if (c == 'a') { x = max(0, x - 1);      move_cnt++; }
	}
